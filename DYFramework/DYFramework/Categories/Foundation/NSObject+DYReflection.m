...
#import "NSObject+DYReflection.h"
#import <objc/runtime.h>

@implementation NSObject (DYReflection)
...
{
    return [[self class] dy_propertyKeys];
}
+ (NSArray *)dy_propertyKeys {
...
{
    return [[self class] dy_propertiesInfo];
}
/**
...
+ (NSArray *)dy_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self dy_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)dy_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] dy_propertiesInfo];
...
-(NSArray*)dy_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] dy_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] dy_decodeType:returnType];
        NSString *encodeString = [[self class] dy_decodeType:encoding];
...
+ (NSArray *)dy_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] dy_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)dy_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)dy_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString dy_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
